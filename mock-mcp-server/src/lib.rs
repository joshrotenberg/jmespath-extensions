...
pub struct MockMcpServer {
    ...
    tool_router: ToolRouter<MockMcpServer>,
}

/// Parameters for the get_discovery_spec tool
#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub struct GetDiscoverySpecParams {}

/// Parameters for generic tool calls
#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub struct GenericToolParams {
...
impl MockMcpServer {
    ...
    pub async fn run(config: ServerConfig) -> anyhow::Result<()> {
        ...
        let service = MockMcpServer::new(config);
...
#[tool_router]
impl MockMcpServer {
    /// Get the discovery spec for this server
    ///
    /// This tool returns the discovery spec that should be registered with jpx
    #[tool(description = "Returns the discovery spec for registering this server's tools with jpx")]
    async fn get_discovery_spec(
        &self,
        #[allow(unused)] Parameters(_params): Parameters<GetDiscoverySpecParams>,
    ) -> Result<CallToolResult, McpError> {
        let spec = self.config.to_discovery_spec();
        Ok(CallToolResult::success(vec![Content::json(&spec)?]))
    }

    /// Echo tool - returns the input (useful for testing)
    #[tool(description = "Echo back the input - useful for testing MCP connectivity")]
    async fn echo(
        &self,
    ...
    /// Server info tool - returns server metadata
    #[tool(description = "Get information about this mock server")]
    async fn server_info(
        ...
        Ok(CallToolResult::success(vec![Content::json(json!({
            ...
            "categories": self.config.categories.iter().map(|c| &c.name).collect::<Vec<_>>(),
        }))?]))
    }
}
